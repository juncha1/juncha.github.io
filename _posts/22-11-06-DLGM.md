...
$\nabla_{\theta}E_{q_{\theta}}[f(\xi)]$ 을 각각 mean과 covariance로 미분하면 아래 식과 같다.
...
식(7)과 식(8)에서 무작위성이 포함되어 있는 $\mu_i$ 와 $C_i$ 를 $\xi$ 로 reparameterization 하면서 unbiased gradient estimate를 가능하게 했다.
그리고 위 두 식을 더하고 chain rule을 사용하여 아래와 같이 gradient를 구할 수 있다.
...
recognition 모델 $q(\xi \vert V)$ 로 인해 EM알고리즘과 같이 반복적인 계산을 수행하는 것이 아닌 single pass 만으로 테스트할 때 빠른 inference를 할 수 있다. 그리고 최고의 inference를 하기 위해서는 $q(\xi \vert V)$ 가 flexible해야할 필요가 있다. 그리고 데이터 input에 bit-flip이나 drop-out과 같은 noise를 추가하여 regularize하면서 모델의 generality를 높였다.
...
이 때 전체 모델 구조에서 검은색 실선 화살표 forward pass를 의미하고 붉은색 화살표는 backward pass를 의미한다.
...
## 8. Opinion
DLGM은 인코더(recognition model)와 디코더(generative model)를 jointly optimize할 수 있게 했고 recognition model을 통해 반복적인 과정 없이 매우 빠른 inference를 가능하게 했다. 하지만 노이즈를 완벽하게 복원해내지 못한다는 변분추론 모델의 공통적인 한계점으로 인해 생성 이미지가 흐릿하다는 단점을 가지고 있다.
